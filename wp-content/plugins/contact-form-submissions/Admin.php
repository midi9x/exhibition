    ...
    public function set_columns($columns)
    {
        ...
        if (isset($_GET['wpcf7_contact_form']) && !empty($_GET['wpcf7_contact_form'])) {
            ...
            foreach ($wpcf7s_columns as $meta_key) {
                ...
                if ($columns[$meta_key] == 'product-id') {
                    $columns[$meta_key] = 'Product';
                }
                $columns[$meta_key] = str_replace('-', ' ', $columns[$meta_key]);
                $columns[$meta_key] = ucwords($columns[$meta_key]);
                $columns[$meta_key] = ucwords($columns[$meta_key]);
    ...
    public function column($column, $post_id)
    {
        ...
        switch ($column) {
            ...
            case 'submission':
                ...
                <?php
                break;
            case 'wpcf7s_posted-product-id':
                $product_id = get_post_meta($post_id, 'wpcf7s_posted-product-id', true);
                ?>
                    <?php echo get_the_title($product_id); ?><br />
                    ( <span>
                        <a href="<?php echo add_query_arg(array('post'=>$product_id, 'action'=>'edit'), admin_url('post.php')); ?>">Edit</a> | 
                        <a target="_blank" href="<?php echo get_permalink($product_id); ?>">View </a>
                    </span>)
                    <?php
                    break;
            default:
