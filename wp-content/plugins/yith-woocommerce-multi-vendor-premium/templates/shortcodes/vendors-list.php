    ...
    <?php foreach( $vendors as $vendor ) :
        ...
        elseif( 'gravatar' == $vendor_image ) {
            if( ! empty( $vendor->avatar ) ){
	            $store_image = wp_get_attachment_image( $vendor->avatar, 150, false, array( 'class' => 'store-image' ) );
            }
            else {
        ...
        <li class="vendor-item <?php echo $vendor->slug; ?>">
            ...
            <div class="vendor-info-wrapper">
                ...
                <ul class="vendor-info<?php echo $show_description ? ' has-description' : '';?>">
                    <?php ! empty( $vendor->location )    && printf( '%s%s%s%s', '<li class="location">', '<i class="fa fa-location-arrow"></i>', $vendor->location, '</li>' ); ?>
                    <?php ! empty( $vendor->store_email ) && printf( '%s%s<a href="mailto:%s">%s</a>%s', '<li class="store-email">', '<i class="fa fa-envelope"></i>', $vendor->store_email, $vendor->store_email, '</li>' ); ?>
                    <?php ! empty( $vendor->telephone )   && printf( '%s%s%s%s', '<li class="telephone">', '<i class="fa fa-phone"></i>', $vendor->telephone, '</li>' ); ?>
