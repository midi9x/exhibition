...
$title          = sprintf( __( 'Report an abuse for product %s', 'yith-woocommerce-product-vendors' ), $product->get_title() );
...
<div id="yith-wpv-abuse-report" style="display: none;">
    ...
    <form action="#" method="post" id="report-abuse" class="report-abuse-form">
        <input type="text" class="input-text " name="report_abuse[name]" value="" placeholder="<?php _e( 'Name', 'yith-woocommerce-product-vendors' ) ?>"  required/>
        <input type="email" class="input-text " name="report_abuse[email]" value="" placeholder="<?php _e( 'Email', 'yith-woocommerce-product-vendors' ) ?>"  required/>
