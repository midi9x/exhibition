				...
				<script type="text/javascript">
					...
					jQuery(function(){
						...
						var drawGraph = function( highlight ) {
							...
							var series = [
								{
									label: "<?php echo esc_js( __( 'Number of commission granted', 'yith-woocommerce-product-vendors' ) ) ?>",
		...
		public function prepare_report_data() {
			...
			foreach ( $vendors as $vendor ) {
				...
				foreach ( $commission_ids as $commision_id ) {
					...
					if( ( 'cancelled' == $commission->status || 'refunded' == $commission->status || 'pending' == $commission->status ) ){
						continue;
					}
