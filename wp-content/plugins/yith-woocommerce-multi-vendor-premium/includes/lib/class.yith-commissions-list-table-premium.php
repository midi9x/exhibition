        ...
        public function get_bulk_actions() {
            ...
            if( $this->_vendor->is_super_user() ){
                ...
                if( 'masspay' == get_option( 'payment_gateway', 'masspay' ) ){
                    $actions = array_merge( array( 'pay' => __( 'Pay', 'yith-woocommerce-product-vendors' ) ), $actions );
                }
            }
        ...
        public function column_user_actions( $rec ) {
            ...
            if ( $this->_vendor->is_super_user() ) {
	            ...
	            if ( $rec->has_status( 'unpaid' ) && 'masspay' == get_option( 'payment_gateway', 'masspay' ) ) {
	                $masspay_uri = esc_url( wp_nonce_url( add_query_arg( array( 'action' => 'pay_commission', 'commission_id' => $rec->id ), admin_url( 'admin.php' ) ), 'yith-vendors-pay-commission' ) );
	                $masspay_uri = apply_filters( 'yith_wcmv_commissions_list_table_masspay_pay_button_url', $masspay_uri, $rec );
